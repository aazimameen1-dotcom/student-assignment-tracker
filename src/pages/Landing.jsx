    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between font-sans select-none animate-fade-in">
      ...
      <header className="max-w-7xl mx-auto w-full px-6 md:px-12 py-6 flex items-center justify-between">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setCurrentView('landing')}>
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#1e1b4b] to-purple-600 flex items-center justify-center text-white shadow-md">
            <span className="material-symbols-outlined text-xl">school</span>
          </div>
          <span className="text-2xl font-extrabold font-headline text-[#1e1b4b] tracking-tight">Assignify</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer"
          >
            Get Started Free
          </button>
        </div>
      ...
      <main className="max-w-7xl mx-auto w-full px-4 md:px-12 py-6 space-y-16">
        ...
        {/* Hero Section */}
        <section className="relative overflow-hidden bg-gradient-to-br from-[#1e1b4b] via-[#231f5c] to-[#312e81] rounded-3xl p-8 md:p-14 text-white shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center relative z-10">
            <div className="col-span-1 md:col-span-7 space-y-6 text-left">
              <span className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full text-xs font-semibold bg-white/10 backdrop-blur-md text-purple-200 border border-white/10 font-mono">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                Stitch Engine Academic System v2.4
              </span>

              <h1 className="text-4xl md:text-5xl font-extrabold font-headline text-white leading-tight tracking-tight">
                Master Your <br />
                <span className="bg-gradient-to-r from-purple-300 to-indigo-200 bg-clip-text text-transparent">Academic Journey</span>
              </h1>
              
              <p className="text-purple-200/90 text-sm md:text-base leading-relaxed max-w-xl">
              ...
              <div className="pt-2 flex flex-wrap items-center gap-4">
                <button 
                  onClick={handleGetStartedClick}
                  className="px-7 py-3.5 bg-white text-[#1e1b4b] hover:bg-slate-100 font-extrabold text-sm rounded-2xl shadow-xl transition-all cursor-pointer flex items-center gap-2"
                >
              ...
              <div className="w-full max-w-md bg-white/10 backdrop-blur-xl p-6 rounded-3xl border border-white/20 shadow-2xl space-y-4">
                ...
                <div className="h-28 relative bg-black/20 rounded-2xl p-4 flex items-end justify-around overflow-hidden border border-white/10">
                  <svg className="absolute inset-0 w-full h-full p-2" viewBox="0 0 200 100" preserveAspectRatio="none">
                    <path d="M 10,80 Q 50,20 90,60 T 180,20" fill="none" stroke="#a78bfa" strokeWidth="4" />
                    <circle cx="10" cy="80" r="5" fill="#38bdf8" />
                    <circle cx="90" cy="60" r="5" fill="#38bdf8" />
                    <circle cx="180" cy="20" r="6" fill="#a78bfa" />
                  </svg>
                </div>

                <div className="flex justify-between items-center text-xs font-semibold pt-2">
                  <span className="text-purple-200">18 Completed Tasks</span>
